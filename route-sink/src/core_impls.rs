...
impl<S: ?Sized + Unpin + FlushRoute<Route, Msg>, Route, Msg> FlushRoute<Route, Msg> for &mut S {
    fn poll_flush_route(
...
impl<S: ?Sized + Unpin + ReadyRoute<Route, Msg>, Route, Msg> ReadyRoute<Route, Msg> for &mut S {
    fn poll_ready_route(
        mut self: Pin<&mut Self>,
        route: &Route,
        cx: &mut Context<'_>,
...
impl<S: ?Sized + Unpin + ReadySome<Route, Msg>, Route, Msg> ReadySome<Route, Msg> for &mut S {
    fn poll_ready_some(
        mut self: Pin<&mut Self>,
        cx: &mut Context<'_>,
    ) -> Poll<Result<Route, Self::Error>> {
